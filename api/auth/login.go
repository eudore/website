...
import (
	...
	"github.com/eudore/website/api/auth/oauth2"
...
type (
	...
	requestLoginAuth struct {
		Username string `bind:"username" json:"username"`
		Password string `bind:"password" json:"password"`
...
func NewLoginController(app *framework.App) *LoginController {
	...
	ctl.loadOauth2(app.App, app.DB)
	return ctl
}

func (ctl *LoginController) loadOauth2(app *eudore.App, db *sql.DB) {
	rows, err := db.Query("SELECT name,server,clientid,clientsecret FROM tb_auth_oauth2_source ORDER BY server,name")
	if err != nil {
		app.Error(err)
		return
	}
...
func (ctl *LoginController) PostWebsite() error {
	...
	if !checkPasswordCompose(auth.Password) {
		return fmt.Errorf("password '%s' to simple", auth.Password)
	}
	// 检测验证码
	...
	var pass string
	var salt []byte
	err = ctl.QueryRow("SELECT id,pass,salt FROM tb_auth_user_pass WHERE name=$1;", auth.Username).Scan(&userid, &pass, &salt)
	if err != nil {
		return err
	}

	// 计算hash
	keys, err := scrypt.Key([]byte(auth.Password), salt, 16384, 8, 1, 32)
	if err != nil {
		return err
	}
	...
	ctl.Infof("login user %s", auth.Username)
...
func (ctl *LoginController) GetOauth2ByName() {
	...
	if ok {
		...
		ctl.SetHeader(eudore.HeaderContentType, eudore.MimeTextPlainCharsetUtf8)
...
func checkPasswordCompose(pass string) bool {
	if pass == "" {
		return false
	}
	ftype := getByteType(pass[0])
	for _, b := range pass {
		if ftype != getByteType(byte(b)) {
			return false
		}
	}
	return true
